...
/**
 * Identifier for a specific application component ({@link android.app.Activity}
 * , {@link android.app.Service}, {@link android.content.BroadcastReceiver}, or
 * {@link android.content.ContentProvider}) that is available. Two pieces of
...
public final class ComponentName implements Comparable<ComponentName> {
     ...
     * 
     * @param pkg The name of the package that the component exists in. Can not
     *            be null.
    ...
    /**
     * Create a new component identifier form a Component object
     */
    public ComponentName(ComponentName name) {
        this.mClass = name.getClassName();
        this.mPackage = name.getPackageName();
    }
     ...
     * names contained in the ComponentName. You can later recover the
     * ComponentName from this string through
     * {@link #unflattenFromString(String)}.
     * 
     * @return Returns a new String holding the package and class names. This is
     *         represented as the package name, concatenated with a '/' and then
     *         the class name.
    ...
    /**
     * The same as {@link #flattenToString()}, but abbreviates the class name if
     * it is a suffix of the package. The result can still be used with
     * {@link #unflattenFromString(String)}.
     * 
     * @return Returns a new String holding the package and class names. This is
     *         represented as the package name, concatenated with a '/' and then
     ...
     * {@link #flattenToString()}. It splits the string at the first '/', taking
     * the part before as the package name and the part after as the class name.
     ...
     * then the final class name will be the concatenation of the package name
     * with the string following the '/'. Thus "com.foo/.Blah" becomes
    ...
    /**
     * Write a ComponentName to a Parcel, handling null pointers. Must be read
     * with {@link #readFromParcel(Parcel)}.
     * 
     ...
     * {@link #writeToParcel(ComponentName, Parcel)}, returning either a null or
     ...
     * previously written with {@link #writeToParcel(Parcel, int)}. Note that
     * you must not use this with data written by
     * {@link #writeToParcel(ComponentName, Parcel)} since it is not possible to
     * handle a null ComponentObject here.
     * 
