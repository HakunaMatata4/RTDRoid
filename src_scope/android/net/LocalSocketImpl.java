...
class LocalSocketImpl
{
    ...
    /**
     * An input stream for local sockets. Needed because we may need to read
     * ancillary data.
     */
    ...
    /**
     * An output stream for local sockets. Needed because we may need to read
     * ancillary data.
     */
     ...
     * The state of the file descriptor is not checked here but the caller can
     * verify socket state by calling listen().
     * 
     ...
     * 
     * @param stream true if this should be a stream socket, false for datagram.
     * @throws IOException
     */
    ...
    /**
     * Accepts a new connection to the socket. Blocks until a new connection
     * arrives.
     * 
    ...
    public void setOption(int optID, Object value)
            throws IOException {
        /*
         * Boolean.FALSE is used to disable some options, so it is important to
         * distinguish between FALSE and unset. We define it here that -1 is
         * unset, 0 is FALSE, and 1 is TRUE.
         */
        int boolValue = -1;
        int intValue = 0;
    ...
    /**
     * Enqueues a set of file descriptors to send to the peer. The queue is one
     * deep. The file descriptors will be sent with the next write of normal
     * data, and will be delivered in a single ancillary message. See
     * "man 7 unix" SCM_RIGHTS on a desktop Linux machine.
     * 
    ...
    /**
     * Retrieves a set of file descriptors that a peer has sent through an
     * ancillary message. This method retrieves the most recent set sent, and
     * then returns null until a new set arrives. File descriptors may only be
     * passed along with regular data, so this method can only return a non-null
