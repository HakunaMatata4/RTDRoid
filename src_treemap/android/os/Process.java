...
public class Process {
     ...
     * function of a <var>processClass</var> is executed there. The process will
     * continue running after this function returns.
     * <p>
     * If processes are not enabled, a new thread in the caller's process is
     * created and main() of <var>processClass</var> called there.
     * <p>
     * The niceName parameter, if not an empty string, is a custom name to give
     * to the process instead of using processClass. This allows you to make
     * easily identifyable processes even if you are using the same base
     ...
     * @param enableDebugger True if debugging should be enabled for this
     *            process.
     * @param zygoteArgs Additional arguments to supply to the zygote process.
     * @return int If > 0 the pid of the new process; if 0 the process is being
     *         emulated by a thread
     * @throws RuntimeException on fatal start failure {@hide}
     */
            ...
            String[] zygoteArgs)
    {
        if (supportsProcesses()) {
            ...
            // Thread constructors must not be called with null names (see
            // spec).
            if (niceName != null) {
    ...
    /**
     * Start a new process. Don't supply a custom nice name. {@hide}
     */
    ...
    /**
     * Tries to open socket to Zygote process if not already open. If already
     * open, does nothing. May block and retry.
     */
    private static void openZygoteSocketIfNeeded()
        ...
        if (sPreviousZygoteOpenFailed) {
            /*
             * If we've failed before, expect that we'll fail again and don't
             * pause for retries.
             */
            retryCount = 0;
        } else {
         ...
         * Really, we'd like to do something better to avoid this condition, but
         * for now just wait a bit...
         */
        for (int retry = 0; (sZygoteSocket == null)
                && (retry < (retryCount + 1)); retry++) {
            ...
            try {
                ...
                sZygoteInputStream = new DataInputStream(
                        sZygoteSocket.getInputStream());
                ...
                if (sZygoteSocket != null) {
                    try {
                        ...
                        Log.e(LOG_TAG,
                                "I/O exception on close after exception",
                                ex2);
                    }
                }
    ...
    private static int zygoteSendArgsAndGetPid(ArrayList<String> args)
        ...
        try {
             ...
             * Presently the wire format to the zygote process is: a) a count of
             * arguments (argc, in essence) b) a number of newline-separated
             * argument strings equal to count After the zygote process reads
     ...
     * @param gids null-ok; a list of supplementary group IDs that the new
    ...
    public static final int getUidForPid(int pid) {
        ...
        Process.readProcLines("/proc/" + pid + "/status", procStatusLabels,
                procStatusValues);
     ...
     *             not have permission to modify the given thread, or to use the
     *             given priority.
     */
     ...
     * 
     * @param tid The identifier of the thread/process to change.
     * @return Returns the current priority, as a Linux priority level, from -20
     *         for highest scheduling priority to 19 for lowest scheduling
     *         priority.
     * @throws IllegalArgumentException Throws IllegalArgumentException if
     *             <var>tid</var> does not exist.
     */
     ...
     * @return Returns true if the underlying system supports this feature, else
     *         false. {@hide}
     */
     ...
     * 
     * @param text The new name of this process. {@hide}
     */
    public static final native void setArgV0(String text);

    /**
     * Kill the process with the given PID. Note that, though this API allows us
     * to request to kill any process based on its PID, the kernel will still
     * impose standard restrictions on which PIDs you are actually able to kill.
     * Typically this means only the process running the caller's
     ...
     *       your own log, or the Android Illuminati will find you some night
     *       and beat you up.
     */
    ...
    public static final native boolean parseProcLine(byte[] buffer,
            int startIndex,
            int endIndex, int[] format, String[] outStrings, long[] outLongs,
            float[] outFloats);
     ...
     * @return the total Pss value for the given process in bytes, or -1 if the
