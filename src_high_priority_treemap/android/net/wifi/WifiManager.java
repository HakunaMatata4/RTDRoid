...
public class WifiManager {
    ...
    @Import
    public static native int getWifiState();

    @Import
    public static native int enableWifi();

    @Import
    public static native int disableWifi();
    ...
    public boolean setWifiEnabled(boolean enabled) {
        ...
        {
            if (getWifiState() == WIFI_STATE_DISABLED)
                return true;
            else
            {
                if (disableWifi() == 0)
...
//
// /**
// * Wi-Fi is currently being disabled. The state will change to {@link
...
// /**
// * Wi-Fi is currently being enabled. The state will change to {@link
...
// * lost. One extra provides the connection state as a boolean, where {@code
// true}
// * means CONNECTED.
...
// * in the form of a {@link android.net.NetworkInfo} object. If the new state
// is
...
// /**
// * The lookup key for a {@link android.net.NetworkInfo} object associated with
// the
...
// * {@link SupplicantState}. Note that the supplicant state is Wi-Fi specific,
// and
// * is not generally the most useful thing to look at if you are just
// interested in
...
// * {@link android.content.Context#getSystemService Context.getSystemService()}
// to retrieve
// * the standard {@link android.content.Context#WIFI_SERVICE
...
// */
// public boolean setWifiApEnabled(WifiConfiguration wifiConfig, boolean
// enabled) {
// try {
...
// * Normally the Wi-Fi radio may turn off when the user has not used the device
// in a while.
// * Acquiring a WifiLock will keep the radio on until the lock is released.
// Multiple
// * applications may hold WifiLocks, and the radio will only be allowed to turn
// off when no
// * WifiLocks are held in any application.
// *
// * Before using a WifiLock, consider carefully if your application requires
// Wi-Fi access, or
// * could function over a mobile network, if available. A program that needs to
// download large
// * files should hold a WifiLock to ensure that the download will complete, but
// a program whose
// * network usage is occasional or low-bandwidth should not hold a WifiLock to
// avoid adversely
// * affecting battery life.
// *
// * Note that WifiLocks cannot override the user-level "Wi-Fi Enabled" setting,
// nor Airplane
// * Mode. They simply keep the radio from turning off when Wi-Fi is already on
// but the device
// * is idle.
// */
...
// *
// * If this WifiLock is reference-counted, each call to {@code acquire} will
// increment the
// * reference count, and the radio will remain locked as long as the reference
// count is
// * above zero.
// *
// * If this WifiLock is not reference-counted, the first call to {@code
// acquire} will lock
// * the radio, but subsequent calls will be ignored. Only one call to {@link
// #release}
// * will be required, regardless of the number of times that {@code acquire} is
// called.
// */
// public void acquire() {
...
// *
// * If this WifiLock is reference-counted, each call to {@code release} will
// decrement the
// * reference count, and the radio will be unlocked only when the reference
// count reaches
// * zero. If the reference count goes below zero (that is, if {@code release}
// is called
// * a greater number of times than {@link #acquire}), an exception is thrown.
// *
// * If this WifiLock is not reference-counted, the first call to {@code
// release} (after
// * the radio was locked using {@link #acquire}) will unlock the radio, and
// subsequent
...
// WifiLock.
// *
// * Reference-counted WifiLocks keep track of the number of calls to {@link
// #acquire} and
// * {@link #release}, and only allow the radio to sleep when every call to
// {@link #acquire}
// * has been balanced with a call to {@link #release}. Non-reference-counted
// WifiLocks
// * lock the radio whenever {@link #acquire} is called and it is unlocked, and
// unlock the
...
// * enough to identify your application and the specific WifiLock within it, if
// it
